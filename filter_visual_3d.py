...
def main():
    ...
    # 保存フォルダ作成
    pre_dirpath = r'C:\Users\AIlab\labo\複合化用NNプログラム_3Dver\\' + date + r'\results\filter_weight\\'
...
def plot_fw_ave(filter_num, kernel_size, target_layer, save_file):
    ...
    sc = ax.scatter(w_x, w_y, w_z, s=100, c=filter_weight, alpha=0.5, cmap='bwr', edgecolors='black')
    # fig.colorbar(sc)
    # plt.savefig(save_file + f"_filter_w_ave.jpg")  # filter_weight
    plt.show()
    # plt.close()
