...
from func_3Dver import load_data_predict, split_num, img_show, pre_concatenate, pre_csv_save
import numpy as np
import os
import pickle
...
def predict_calc_save(pre_file, t_v, result_dirpath, EPOCHS, model):
    ...
    os.makedirs(pre_dirpath + "save_csv", exist_ok=True)
    ...
    pre_csv_save(x_pre_total, pre_dirpath + f'save_csv\\pre_{pre_file}_dataset={dataset_num}_e={EPOCHS}_b={BATCH_SIZE}')
