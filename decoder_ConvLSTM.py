from func_3DCNN import load_dataset, date, save_data_csv, save_data_txt, data_num, plot_history, hist_csv_save, \
    dirname_main, spike_data_num
from func_model import model_build, ssim_loss, model_ConvLSTM2D, model_cnn_LSTM
...
def model3D(x_trains, y_trains, unused_filename, dataset_num, time_size, xy_size, filter_size, result_date_dirpath):
    ...
    # model = model_ConvLSTM2D(time_size, xy_size, filter_size, input_shape)
    model = model_cnn_LSTM(time_size, xy_size, filter_size, input_shape)
    print("test2")
    # 打ち切り設定
