...
def main():
    # 学習済みmodelの読み込み
    dirname = r'C:\Users\AIlab\labo\複合化用NNプログラム_3Dver\\' + date + r'\results_k100\model'
    model_name = dirname + f'\model_dataset={dataset_num}_e={EPOCHS}_b={BATCH_SIZE}.h5'
    model = load_model(model_name)
    model.summary()

    # 保存フォルダ作成
    pre_dirpath = r'C:\Users\AIlab\labo\複合化用NNプログラム_3Dver\\' + date + r'\results_k100\filter_weight\\'
...
def plot_fw_one(filter_num, kernel_size, target_layer, save_file):
    for j in range(filter_num):
        ...
        # アスペクト比変更
        ax.set_box_aspect((50, 5, 5))

        sc = ax.scatter(w_x, w_y, w_z, s=100, c=filter_weight, alpha=0.5, cmap='bwr', edgecolors='black')
        # fig.colorbar(sc)
